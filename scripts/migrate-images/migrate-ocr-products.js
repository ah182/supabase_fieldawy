 ...
 * Target: ocr_products table (image_url field)
 * Bucket: ocr
 * Folder: products
 */
    ...
    return new Promise((resolve, reject) => {
        ...
        const request = protocol.get(url, options, (response) => {
            ...
            if (response.statusCode !== 200) {
                // If 404/401, we want to know, but also reject so we can try signed URL
...
async function migrateProduct(record) {
    const { id, image_url, product_name } = record;

    console.log(`\n💊 Processing Product: ${product_name || id}`);
    console.log(`   ID: ${id}`);
    ...
    try {
        ...
        try {
            ...
            // Include logic for retrying with authentication
...
async function migrate() {
    console.log('═══════════════════════════════════════════════════════════');
    console.log('🚀 Starting Migration: OCR Products Images → Supabase Storage');
    console.log('═══════════════════════════════════════════════════════════');
    ...
    console.log('📊 Fetching products from ocr_products...');
    ...
    console.log(`💊 Found ${stats.total} products with images\n`);

    for (const record of records) {
        await migrateProduct(record);
