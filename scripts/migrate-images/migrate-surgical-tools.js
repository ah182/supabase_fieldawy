 ...
 * Target: surgical_tools table (image_url field)
 * Bucket: ocr
 * Folder: surgical_tools
 */
...
const FOLDER_NAME = 'surgical_tools';
...
/**
 * Migrate a single tool image
 */
async function migrateTool(record) {
    const { id, image_url, tool_name } = record;

    console.log(`\n✂️ Processing Tool: ${tool_name || id}`);
    console.log(`   ID: ${id}`);
...
async function migrate() {
    console.log('═══════════════════════════════════════════════════════════');
    console.log('🚀 Starting Migration: Surgical Tools Images → Supabase Storage');
    console.log('═══════════════════════════════════════════════════════════');
    ...
    console.log('📊 Fetching tools from surgical_tools...');

    let query = supabase
        .from('surgical_tools')
        .select('id, image_url, tool_name')
    ...
    console.log(`✂️ Found ${stats.total} tools with images\n`);

    for (const record of records) {
        await migrateTool(record);
