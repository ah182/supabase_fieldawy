 ...
 * Target: vet_courses table (image_url field)
 * Bucket: ocr
 * Folder: courses
 */
...
function extractPublicId(url) {
    try {
        ...
        if (path.startsWith('v') && path.includes('/')) {
            ...
            // Check if it's really a version number like v123
...
function getAuthenticatedUrl(originalUrl) {
    ...
    // Try generating a signed URL with 'authenticated' type
...
/**
 * Migrate a single course image
 */
async function migrateCourse(record) {
    const { id, image_url, title } = record;

    console.log(`\n📚 Processing Course: ${title || id}`);
    console.log(`   ID: ${id}`);
    ...
    try {
        ...
        try {
            ...
            if (e.message.includes('401') || e.message.includes('403')) {
                console.log('   🔒 Access denied, trying signed URL...');
                ...
                try {
                    ...
                    console.log('   🔒 Trying standard signed URL...');
...
async function migrate() {
    console.log('═══════════════════════════════════════════════════════════');
    console.log('🚀 Starting Migration: Vet Courses Images → Supabase Storage');
    console.log('═══════════════════════════════════════════════════════════');
    ...
    console.log('📊 Fetching courses from vet_courses...');
    ...
    console.log(`📚 Found ${stats.total} courses with images\n`);

    for (const record of records) {
        await migrateCourse(record);
