...
BEGIN
    ...
    VALUES (
        ...
        v_product_package,
        v_user_id,
        NULLIF(TRIM(p_request_comment), ''),  -- حفظ التعليق (NULL إذا كان فارغاً)
        'active'
    )
...
-- 4. تحديث دالة get_active_review_requests لإرجاع request_comment
...
RETURNS TABLE (
    ...
    request_comment TEXT  -- إضافة حقل التعليق
)
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
    ...
    SELECT 
        ...
        rr.request_comment  -- إضافة حقل التعليق
...
DO $$
BEGIN
    RAISE NOTICE '✅ تم إضافة حقل request_comment إلى جدول review_requests';
    RAISE NOTICE '✅ تم تحديث دالة create_review_request لقبول التعليق';
    RAISE NOTICE '✅ تم تحديث دالة get_active_review_requests لإرجاع التعليق';
END $$;
