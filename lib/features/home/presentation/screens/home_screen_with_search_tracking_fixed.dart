...
class HomeScreenWithSearchTrackingFixed extends ConsumerStatefulWidget {
  final int? initialTabIndex;
  final String? distributorId;
  
  const HomeScreenWithSearchTrackingFixed({
    super.key,
  ...
  @override
  ConsumerState<HomeScreenWithSearchTrackingFixed> createState() => _HomeScreenWithSearchTrackingFixedState();
}

class _HomeScreenWithSearchTrackingFixedState extends ConsumerState<HomeScreenWithSearchTrackingFixed>
  ...
  void initState() {
    ...
    // ✅ إعداد debounced search مع تتبع البحث
    _searchController.addListener(() {
      setState(() {
        _searchQuery = _searchController.text;
      });

      // تطبيق debouncing للبحث مع تتبع
      Future.delayed(const Duration(milliseconds: 800), () {
        if (_searchController.text == _searchQuery) {
          ...
          // ✅ تسجيل عملية البحث عند التغيير
          _trackSearchIfNeeded(_searchController.text);
        }
      });
    });
    ...
    // ✅ مزامنة الفهرس المحدد مع provider (تم إصلاح الاسم)
  ...
  // ✅ دالة تتبع البحث الجديدة
  Future<void> _trackSearchIfNeeded(String searchTerm) async {
    // تجنب تتبع البحث الفارغ أو المكرر
    if (searchTerm.trim().isEmpty || searchTerm == _lastSearchTerm) {
      return;
    }
    ...
    try {
      ...
      print('🔍 Search tracked: "$searchTerm" in $searchType tab ($resultCount results)');
    } catch (e) {
            ...
            title: Container(
              ...
              child: TextField(
                ...
                onSubmitted: (value) async {
                  _focusNode.unfocus();
                  await _trackSearchIfNeeded(value);
                },
                onChanged: (value) {
                  setState(() {
                    _searchQuery = value;
